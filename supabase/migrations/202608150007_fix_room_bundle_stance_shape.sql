-- PostgreSQL resolves an unqualified row alias that matches a column name to
-- the scalar column. The original `to_jsonb(stance)` therefore serialized
-- `["challenge"]` instead of stance row objects once the first stance existed.
-- Keep the room bundle atomic while using an unambiguous row alias.
