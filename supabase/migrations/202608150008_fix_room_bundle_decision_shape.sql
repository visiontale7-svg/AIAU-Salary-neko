-- As with node stances, PostgreSQL resolves an unqualified row alias that
-- matches a column name to the scalar column. `to_jsonb(decision)` therefore
-- serialized decision enum values rather than complete decision rows.
-- Use unambiguous aliases for both affected aggregates.
